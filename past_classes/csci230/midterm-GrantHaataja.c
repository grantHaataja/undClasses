...
struct _data *LOAD(FILE *stream, int size, char *fileName) {
	...
	//to store number of characters in each line
	ssize_t read = 0;
	//string variable to hold first name from each line and copy it to BlackBox
	char *first = NULL;
	//string variable to hold last name from each line and copy it to BlackBox
	char *second = NULL;	
	...
	for (int i = 0; i < size; i++) {
		...
		//use deliminator to get first name from the line and assign to 'first'
		first = strtok(line, " ,\n");
		//allocate memory for each first name in the struct
		BlackBox[i].firstName = (char*) calloc(strlen(first), sizeof(char));
		//copy each first name into the struct
		strncpy(BlackBox[i].firstName, first, (int) strlen(first));
		//use deliminator to get last name from the line and assign to 'second'
		second = strtok(NULL, " ");
		//allocate memory for each last name in the struct
		BlackBox[i].lastName = (char*) calloc(strlen(second), sizeof(char));
		//copy each last name into the struct
		strncpy(BlackBox[i].lastName, second, (int) strlen(second));					
		//use deliminator to get the number from the line and assign to 'third'
		third = strtok(NULL, " ");
		//convert string to long and store in struct
		BlackBox[i].number = atol(third);
		//free line variable
		free(line);
	}
	return BlackBox;
...
void SEARCH(struct _data *BlackBox, char *name, int size) {
	...
	for (int i = 0; i < size; i++) {
		if (strcmp(BlackBox[i].firstName, name) == 0) {
			//print the found name
			printf("\nThe name %s was found at the %d entry.\n", name, i+1);
			found = 1;
			break;
		}
		else if (strcmp(BlackBox[i].lastName, name) == 0) {
			//print the found name
			printf("\nThe name %s was found at the %d entry.\n", name, i+1);
			found = 1;
			break;
